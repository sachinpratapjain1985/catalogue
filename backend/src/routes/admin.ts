...
export const applyWatermark = async (inputFilePath: string, outputFilePath: string) => {
  try {
    ...
    const svgOverlay = Buffer.from(`<?xml version="1.0" encoding="UTF-8"?>
      <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
        <g transform="rotate(-30 ${width / 2} ${height / 2})">
          <text x="${width / 2 + 2}" y="${centerY + 2}" text-anchor="middle" fill="rgba(0, 0, 0, 0.25)" font-size="${fontSize}px" font-family="Arial, sans-serif" font-weight="900" letter-spacing="3px">${text}</text>
          <text x="${width / 2}" y="${centerY}" text-anchor="middle" fill="rgba(255, 255, 255, 0.45)" font-size="${fontSize}px" font-family="Arial, sans-serif" font-weight="900" letter-spacing="3px">${text}</text>
        </g>
      </svg>
    `);
    ...
    console.log(`[Watermark] Successfully applied single light 100% quality VS FASHION (DESUKA) watermark to ${outputFilePath}`);
  } catch (err) {
